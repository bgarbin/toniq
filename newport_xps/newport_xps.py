...
class Device():
    ...
    def __init__(self,address='192.168.0.4',port=5001):
...
class NSR1():
    ...
    def getDataPath(self):
        if __name__ == '__main__' :
            ...
            return os.path.join(os.path.dirname(os.path.realpath(__file__)),self.SLOT)
        
    def loadCalib(self):
        dataPath = self.getDataPath()
        calibAngle = np.load(os.path.join(os.path.join(dataPath),'calibAngle.npy'))
        calibTransmission = np.load(os.path.join(os.path.join(dataPath),'calibTransmission.npy'))
        calibPower = np.load(os.path.join(os.path.join(dataPath),'calibPower.npy'))
        d = {'angle':calibAngle,'transmission':calibTransmission,'power':calibPower}
        self.calib = pd.DataFrame(d)
