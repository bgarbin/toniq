...
class Device():
        ...
        def get_data(self,chan='CH1',filename='test_save_file_',PLOT=False,typ='BYTE',SAVE=False,LOG=True):
            ...
            if SAVE:
                ...
                f = open(filename + '_DSO54853A' + chan,'w')                   # Save data
                f.write(self.data)
                f.close()
                if LOG:
                    self.preamb = self.get_log_data(chan=chan)             # Save scope configuration
                    f = open(filename + '_DSO54853A' + chan + '.log','w')
                    f.write(self.preamb)
                    f.close()
                print(chan + ' saved')
...
if __name__ == '__main__':
    ...
    ### Start the talker ###
    Device(channel=chan,host=options.ipadd,query=options.que,command=options.com,filename=options.filename,typ=options.type)
